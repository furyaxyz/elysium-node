...
const EnvCustomNetwork = "ELYSIUM_CUSTOM"
...
func parseNetworkFromEnv() (Network, error) {
	...
	// format: ELYSIUM_CUSTOM=<netID>:<genesisHash>:<bootstrapPeerList>
	if custom, ok := os.LookupEnv(EnvCustomNetwork); ok {
		fmt.Print("\n\nWARNING: Elysium custom network specified. Only use this option if the node is " +
