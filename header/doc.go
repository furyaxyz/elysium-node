...
An ExtendedHeader is similar to a regular block header from elysium-core, but "extended" by adding
...
ExtendedHeaders are generated by bridge nodes that listen to the elysium-core network for blocks,
validate and extend the block data in order to generate the DAH, and request both the ValidatorSet
and Commit for that block height from the elysium-core network. Bridge nodes then package that data
...
There are 5 main components in the header package:
 1. core.Listener listens for new blocks from the elysium-core network (run by bridge nodes only),
    extends them, generates a new ExtendedHeader, and publishes it to the HeaderSub.
 2. p2p.Subscriber listens for new ExtendedHeaders from the Elysium Data Availability (DA) network (via
    the HeaderSub)
 3. p2p.Exchange or core.Exchange request ExtendedHeaders from other elysium DA nodes (default for
    full and light nodes) or from a elysium-core node connection (bridge nodes only)
 4. Syncer manages syncing of past and recent ExtendedHeaders from either the DA network or a elysium-core
...
For bridge nodes, the general flow of the header Service is as follows:
 ...
 4. Syncer is already subscribed to the HeaderSub, so it receives new ExtendedHeaders locally from
    ...
    - If the elysium-core connection is started simultaneously with the bridge node, then the elysium-core
    connection will handle the syncing component, piping every synced block to the core.Listener
    - If the elysium-core connection is already synced to the network, the Syncer handles requesting past
    headers up to the network head from the elysium-core connection (using core.Exchange rather than p2p.Exchange).
