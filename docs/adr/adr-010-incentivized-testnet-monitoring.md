...
- 2022-7-19: Started
- 2022-7-22: Add section on "How to monitor elysium-node with Grafana Cloud"
- 2022-7-26: Add section on "How to monitor elysium-node with Uptrace"
- 2022-7-29: Add section on "How to send data over HTTPS"
- 2022-8-1: Revise architecture to minimize Elysium managed components
- 2022-8-4: Add section on "Why doesn't the Elysium team host OTEL Collectors for node operators?"
...
We're adding telemetry to elysium-node by instrumenting our codebase with metrics (see [ADR-009-telemetry](./adr-009-telemetry.md)). If the option to report metrics is enabled on elysium-node, then elysium-node will push metrics via [OTLP Exporter](https://opentelemetry.io/docs/reference/specification/protocol/exporter/) to an [OTEL Collector](https://opentelemetry.io/docs/collector/) instance.

We would like to make the metrics exported by elysium-node actionable by making them queryable in internal Grafana dashboards. We additionally want a subset of metrics to be queryable by a public incentivized testnet leaderboard frontend.
...
<details>
<summary> How to monitor elysium-node with Grafana Cloud</summary>

1. [Install elysium-node](https://docs.elysium.org/developers/elysium-node)
2. Sign up for an account on [Grafana](https://grafana.com/)
3. [Install OTEL Collector](https://opentelemetry.io/docs/collector/getting-started/) on the same machine as elysium-node. If on a Linux machine follow [these steps](https://opentelemetry.io/docs/collector/getting-started/#linux-packaging=). OTEL Collector should start automatically immediately after installation.
4. Configure OTEL Collector to receive metrics from elysium-node by confirming your `/etc/otelcol/config.yaml` has the default config:
    ...
    This starts the [OTLP receiver](https://github.com/open-telemetry/opentelemetry-collector/blob/main/receiver/otlpreceiver/README.md) on port 4317 for gRPC and 4318 for HTTP. Elysium-node will by default emit HTTP metrics to `localhost:4318` so if you deployed OTEL Collector on the same machine as elysium-node, you can preserve the default config.
...
9. Start elysium-node with metrics enabled `elysium light start --core.ip https://rpc-mamaki.pops.one --metrics`
...
<details>
<summary> How to monitor elysium-node with Uptrace</summary>

1. [Install elysium-node](https://docs.elysium.org/developers/elysium-node).
2. Create an account on [Uptrace](https://app.uptrace.dev/).
3. Create a project on Uptrace.
4. Follow [these steps](https://uptrace.dev/opentelemetry/collector.html#when-to-use-opentelemetry-collector=) to install OTEL Collector Contrib on the same host as elysium-node.
...
7. Start elysium-node with metrics and traces enabled: `elysium light start --core.ip https://rpc-mamaki.pops.one --tracing --metrics`.
...
#### Scenario B: Elysium team

![scenario b](./img/incentivized-testnet-monitoring-scenario-b.png)

Pros

- It will be easier for nodes to participate if they only have to deploy one piece of software (elysium-node) and not two (elysium-node and OTEL Collector).
...
#### Scenario C: Node operators by default and Elysium team as a fallback

![scenario c](./img/incentivized-testnet-monitoring-scenario-c.png)

Pros

- Optionality for node operators who don't want to deploy an OTEL Collector to rely on a best-effort OTEL Collector provided by Elysium team.

Cons

- This option increases the cognitive load on node operators who now have an additional decision at deployment time.
- Increased operational burden on Elysium team during incentivized testnet (and beyond).

#### Scenario D: Elysium team by default and node operators if they want

The diagram, pros, and cons are the same as scenario C.

This scenario differs from Scenario C in the docs for node deployment. The docs specify a Elysium team managed OTEL Collector endpoint by default. For node operators who want self-managed telemetry, the docs contain steps on how to setup a node operator managed agent OTEL Collector and how to proxy metrics to the Elysium team managed gateway OTEL Collector.
...
We agreed on **Scenario D** at this time. Our docs will describe how to connect to the Elysium team managed OTEL Collector (i.e. `--metrics.endpoint`). Our docs will also include an advanced guide for node operators if they wish to deploy a self-managed OTEL Collector.

### Should node operators be able to configure elysium-node to export to multiple OTEL collectors?
...
#### elysium-node -> OTEL Collector with public certificate

In the case where an OTEL Collector is running on a different host than elysium-node, then the OTEL Collector must be configured with a public certificate so that elysium-node can send data to it over HTTPS.

1. Ensure that elysium-node doesn't use [`WithInsecure`](https://github.com/open-telemetry/opentelemetry-go/blob/main/exporters/otlp/otlpmetric/otlpmetrichttp/options.go#L161) when constructing otlptracehttp client
...
receivers:
  otlp:
    protocols:
      grpc:
      http:
        endpoint: otel.collector.elysium.observer:4318
        tls:
          cert_file: /home/fullchain.pem
          key_file:  /home/privkey.pem
```

#### elysium-node -> OTEL Collector without public certificate

In the case where a node operator wants to send data from elysium-node to an OTEL Collector without a public certificate (e.g. node-operator managed OTEL Collector), they can issue self-signed certificate in order send data over HTTPS. Alternatively they can send data over HTTP.
