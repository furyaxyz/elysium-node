...
The central data structure representing Elysium block data is EDS(`rsmt2d.ExtendedDataSquare`), and the new storage design
...
- _The Blockstore does not store whole Elysium Blocks, but IPFS blocks. We represent Merkle proofs and shares in IPFS
  blocks._
...
// registered on the Store. NOTE: The Blockstore does not store whole Elysium Blocks but IPFS blocks. 
...
- _The returned Blockstore does not store whole Elysium Blocks, but IPFS blocks. We represent Merkle proofs and shares in IPFS
  blocks._

```go
// CARBlockstore returns an IPFS Blockstore providing access to individual shares/nodes of a specific EDS identified by 
// DataHash and registered on the Store. NOTE: The Blockstore does not store whole Elysium Blocks but IPFS blocks. 
// We represent `shares` and NMT Merkle proofs as IPFS blocks and IPLD nodes so Bitswap can access those.
