# ADR #003: March 2022 Testnet Elysium Node
...
A **bridge** node is a node that is connected to a elysium-core node via RPC. It receives a remote address from a
running elysium-core node and listens for blocks from elysium-core. For each new block from elysium-core, the **bridge**
...
A **bridge** node does not care about what kind of elysium-core node it is connected to (validator or regular full node),
it only cares that it has a direct RPC connection to a elysium-core node from which it can listen for new blocks.

The name **bridge** was chosen as the purpose of this node type is to provide a mechanism to relay elysium-core blocks
...
This ADR describes a design for the March 2022 Elysium Testnet that we decided at the Berlin 2021 offsite. Now that
we have a basic scaffolding and structure for a elysium node, the focus of the next engineering sprint is to continue
...
some problems with regards to sync (we initially made the decision that a elysium full node would have to be started
...
work otherwise, leading to last-minute solutions, like having to hand both the elysium **light** and **full** node a
