...
Elysium Node has been built for almost half a year with a bottom-up approach to
...
[here](https://github.com/elysiumorg/elysium-node/issues/944). It provides a
more detailed step-by-step process for how the below described design will be
implemented.

### High-level description

All node types in `elysium-node` will be referred to as `data availability
...
This means that DA nodes will be able to query for / modify elysium state such
...
Furthermore, interaction between the elysium consensus network and the
elysium data availability network will be the responsibility of the **bridge**
...
elysium-core node to which it is connected). A **bridge** node, for all intents
...
- Consistency. Semantically, modules are closer to what Elysium's overarching
...
```go
  type StateModule interface {
    ...
    // BalanceForAddress retrieves the Elysium coin balance for the given 
    ...
    // SubmitTx submits the given transaction/message to the Elysium network
